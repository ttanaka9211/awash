...
$s = $_GET['s'];
$custompost_cat = $_GET['custompost_cat'];
$custompost_tag = $_GET['custompost_tag'];
...
if (isset($post_cat)) :
    $taxquery_cat = array(
        'taxonomy' => $custompost_cat,
        'terms' => $post_cat, //選択されたタームを取得
        'field' => 'slug',
...
$args = array(
    'post_type' => $post_type,
    's' => $s,
    'orderby' => $orderby, // デフォルト値'date' 複数のオプションを渡すことが可能
    'order' => $order, // 'ASC'昇順  'DESC'降順
    'tax_query' => array(
        'relation' => 'AND',
        $taxquery_cat,
        $taxquery_tag
